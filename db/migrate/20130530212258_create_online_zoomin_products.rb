class CreateOnlineZoominProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,trial_flag,created_at) values('zoomin','/Images/Vendor_Logos/infibeam.png','An Online Shop','online','www.zoomin.com','feedback@infibeam.in','08049091234','na', '9th Floor,A-Wing,Gopal Palace,Nehrunagar,Ahmedabad. Gujarat. India - 380015.','na','na','na','na','na','COD',
'mobiles_lists','purchase','2012-07-07','n',now());

	SQL

  execute <<-SQL

  CREATE TABLE IF NOT EXISTS online_zoomin_products (
  ...
  SQL
    ...
    CREATE TRIGGER t_Insert_online_zoomin_products AFTER INSERT ON online_zoomin_products
    FOR EACH ROW
    BEGIN
	...
	SELECT vendor_id INTO v_VendorID FROM vendors_lists WHERE vendor_name='zoomin';
    ...
    CREATE TRIGGER t_Update_online_zoomin_products AFTER UPDATE ON online_zoomin_products
    FOR EACH ROW
    BEGIN
...
SET v_DebugID = concat('uonlinev',v_VendorID,'p',new.product_id);
  ...
  call p_insert_update_delete_products_table(  v_delete_flag,
    ...
    CREATE TRIGGER t_Delete_online_zoomin_products BEFORE DELETE ON online_zoomin_products
    FOR EACH ROW
    BEGIN
 ...
 call p_insert_update_delete_products_table(    v_delete_flag,
  ...
  def down
	...
	execute "DELETE FROM online_zoomin_products"
	execute "call p_drop_online_vendor('zoomin')"
        execute "DROP TRIGGER t_Delete_online_zoomin_products"
        execute "DROP TRIGGER t_Update_online_zoomin_products"
        execute "DROP TRIGGER t_Insert_online_zoomin_products"
        execute "DROP TABLE online_zoomin_products"
        system "rails destroy model online_zoomin_products"
  end
end
