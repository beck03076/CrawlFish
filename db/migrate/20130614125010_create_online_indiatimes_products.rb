class CreateOnlineIndiatimesProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,trial_flag,created_at) values('IndiaTimes','/Images/Vendor_Logos/infibeam.png','An Online Shop','online','www.infibeam.com','feedback@infibeam.in','08049091234','na', '9th Floor,A-Wing,Gopal Palace,Nehrunagar,Ahmedabad. Gujarat. India - 380015.','na','na','na','na','na','COD',
'desktops_lists,headsets_lists,laptops_lists,pendrives_lists,keyboards_lists,headphones_lists,printers_lists,tablets_lists,routers_lists,mouses_lists,speakers_lists','purchase','2012-07-07','n',now());
  ...
  CREATE TABLE IF NOT EXISTS online_indiatimes_products (
  ...
  SQL
    ...
    CREATE TRIGGER t_Insert_online_indiatimes_products AFTER INSERT ON online_indiatimes_products
    FOR EACH ROW
    BEGIN
	...
	SELECT vendor_id INTO v_VendorID FROM vendors_lists WHERE vendor_name='indiatimes';
    ...
    CREATE TRIGGER t_Update_online_indiatimes_products AFTER UPDATE ON online_indiatimes_products
    FOR EACH ROW
    BEGIN
...
SET v_DebugID = concat('uonlinev',v_VendorID,'p',new.product_id);
  ...
  call p_insert_update_delete_products_table(  v_delete_flag,
    ...
    CREATE TRIGGER t_Delete_online_indiatimes_products BEFORE DELETE ON online_indiatimes_products
    FOR EACH ROW
    BEGIN
 ...
 call p_insert_update_delete_products_table(    v_delete_flag,
  ...
  def down
	...
	execute "DELETE FROM online_indiatimes_products"
	execute "call p_drop_online_vendor('indiatimes')"
        execute "DROP TRIGGER t_Delete_online_indiatimes_products"
        execute "DROP TRIGGER t_Update_online_indiatimes_products"
        execute "DROP TRIGGER t_Insert_online_indiatimes_products"
        execute "DROP TABLE online_indiatimes_products"
        system "rails destroy model online_indiatimes_products"
  end
end
