class CreateOnlineSnapdealProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,trial_flag,created_at) values('Snapdeal','/Images/Vendor_Logos/croma.png','An Online Shop','online','www.croma.com','feedback@gcroma.in','02131230324324','na', 'greendust,Nehrunagar,Ahmedabad. Gujarat. India - 380015.','na','na','na','na','na','COD',
'printers_lists,tablets_lists,routers_lists','purchase','0','n',now());

	SQL

  execute <<-SQL

  CREATE TABLE IF NOT EXISTS online_snapdeal_products (
  ...
  SQL
    ...
    CREATE TRIGGER t_Insert_online_snapdeal_products AFTER INSERT ON online_snapdeal_products
    FOR EACH ROW
    BEGIN
	...
	SELECT vendor_id INTO v_VendorID FROM vendors_lists WHERE vendor_name='snapdeal';
    ...
    CREATE TRIGGER t_Update_online_snapdeal_products AFTER UPDATE ON online_snapdeal_products
    FOR EACH ROW
    BEGIN
...
SET v_DebugID = concat('uonlinev',v_VendorID,'p',new.product_id);
  ...
  call p_insert_update_delete_products_table(  v_delete_flag,
    ...
    CREATE TRIGGER t_Delete_online_snapdeal_products BEFORE DELETE ON online_snapdeal_products
    FOR EACH ROW
    BEGIN
 ...
 call p_insert_update_delete_products_table(    v_delete_flag,
  ...
  def down
	...
	execute "DELETE FROM online_snapdeal_products"
	execute "call p_drop_online_vendor('snapdeal')"
        execute "DROP TRIGGER t_Delete_online_snapdeal_products"
        execute "DROP TRIGGER t_Update_online_snapdeal_products"
        execute "DROP TRIGGER t_Insert_online_snapdeal_products"
        execute "DROP TABLE online_snapdeal_products"
        system "rails destroy model online_snapdeal_products"
  end
end
