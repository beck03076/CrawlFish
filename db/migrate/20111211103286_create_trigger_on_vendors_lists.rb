class CreateTriggerOnVendorsLists < ActiveRecord::Migration
  def up
	...
	INSERT INTO vendors_details (vendor_id, vendor_name, vendor_city, vendor_branch, created_at)
	VALUES (new.vendor_id, new.vendor_alias_name, new.city_name, new.branch_alias_name, CURRENT_TIMESTAMP);

	/* Insert a record in debug table for tracking the events */
        call debug.debug_insert(v_DebugID,concat('A record inserted into vendor_details table for vendor_id, ',new.vendor_id));
	...
	IF new.monetization_type = "fixed" THEN
		...
		UPDATE fixed_pay_vendors SET history_flag = 0, accepted_amount=0, period=0, subscribed_date = new.subscribed_date, 
		...
		INSERT INTO fixed_pay_vendors(vendor_id,accepted_amount,period,subscribed_date,cut_off_period,created_at)
		VALUES(new.vendor_id,0,0,new.subscribed_date,0,now());
 ...
 /* Ending the debug table insert with a #(pound) mark */
    ...
    DECLARE v_subscribed_date DATETIME;

    /*After declaring v_DebugID, the debug_on procedure is called which is insert a record in debug table */
    call debug.debug_on(v_DebugID);


	UPDATE vendors_details SET vendor_name = new.vendor_alias_name,
				   vendor_city = new.city_name,
				   vendor_branch = new.branch_alias_name,
				   updated_at = CURRENT_TIMESTAMP
	WHERE vendor_id = new.vendor_id;

	/* Insert a record in debug table for tracking the events */
        call debug.debug_insert(v_DebugID,concat('Record updated in vendors_details table for vendor_id ',new.vendor_id));
	...
	IF new.monetization_type = "fixed" THEN
		SELECT COUNT(*), subscribed_date INTO v_fp_count, v_subscribed_date FROM fixed_pay_vendors
		...
		INSERT INTO fixed_pay_vendors(vendor_id,accepted_amount,period,subscribed_date,cut_off_period,created_at)
		VALUES(old.vendor_id,0,0,new.subscribed_date,0,now());
		...
		ELSEIF v_fp_count = 1 AND v_subscribed_date = new.subscribed_date THEN

		/* Insert a record in debug table for tracking the events */
                call debug.debug_insert(v_DebugID,concat('No change made to fixed_pay_vendors table for the vendor ',old.vendor_name,' ',old.vendor_id));

		ELSE

		UPDATE fixed_pay_vendors SET subscribed_date = new.subscribed_date,accepted_amount=0,period=0,
	...
	ELSEIF new.monetization_type = "variable" THEN
		...
		SELECT COUNT(*), subscribed_date INTO v_vp_count, v_subscribed_date FROM variable_pay_vendors
		WHERE vendor_id = old.vendor_id;
		...
		ELSEIF v_vp_count = 1 AND v_subscribed_date = new.subscribed_date THEN

		/* Insert a record in debug table for tracking the events */
                call debug.debug_insert(v_DebugID,concat('No change made to variable_pay_vendors table for the vendor ',old.vendor_name,' ',old.vendor_id));
	...
	ELSEIF new.monetization_type = "purchase" THEN
		...
		SELECT count(*) INTO v_ppc_count FROM product_purchase_commission_vendors
		WHERE vendor_id = new.vendor_id AND subscribed_date = new.subscribed_date;
    ...
    DELETE FROM vendors_details WHERE vendor_id = old.vendor_id;

    /* Insert a record in debug table for tracking the events */
    call debug.debug_insert(v_DebugID,concat('Deleted a record in vendors_details table for vendor_id ',old.vendor_id));
