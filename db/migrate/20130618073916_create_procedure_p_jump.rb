class CreateProcedurePJump < ActiveRecord::Migration
  def up
  execute "DROP PROCEDURE IF EXISTS p_jump"

    execute <<-SQL
    CREATE PROCEDURE p_jump (IN v_cols VARCHAR(255),
                        IN p_cat VARCHAR(50),
                        IN p_bran TEXT,
...
BEGIN
     ...
     DECLARE p_id,p_url,p_name,p_id1,p_id2 VARCHAR(500);
     DECLARE d_id2 TEXT;     
     ...
     SET @query = CONCAT('CREATE VIEW t_view as select ',v_cols, ' from ',p_cat,' where ',p_bran_name,' IN (',p_bran,')'); 
     select @query;
     ...
     OPEN cur_products; 
        FETCH cur_products INTO p_id,p_url,p_name,p_id1,p_id2;
     ...
     SET @take = @diff * @v_c;
     SELECT CONCAT('It took ',@take,' seconds to process ',@v_c, ' records..');
