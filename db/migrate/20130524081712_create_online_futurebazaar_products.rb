class CreateOnlineFuturebazaarProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,trial_flag,created_at) values('Futurebazaar','/Images/Vendor_Logos/croma.png','An Online Shop','online','www.croma.com','feedback@gcroma.in','02131230324324','na', 'greendust,Nehrunagar,Ahmedabad. Gujarat. India - 380015.','na','na','na','na','na','COD',
'external_hdds_lists,laptops_lists,memory_cards_lists,pendrives_lists,headphones_lists,mobiles_lists,tablets_lists,routers_lists,mouses_lists,speakers_lists','purchase','0','n',now());

	SQL

  execute <<-SQL

  CREATE TABLE IF NOT EXISTS online_futurebazaar_products (
  ...
  SQL
    ...
    CREATE TRIGGER t_Insert_online_futurebazaar_products AFTER INSERT ON online_futurebazaar_products
    FOR EACH ROW
    BEGIN
	...
	SELECT vendor_id INTO v_VendorID FROM vendors_lists WHERE vendor_name='futurebazaar';
    ...
    CREATE TRIGGER t_Update_online_futurebazaar_products AFTER UPDATE ON online_futurebazaar_products
    FOR EACH ROW
    BEGIN
...
SET v_DebugID = concat('uonlinev',v_VendorID,'p',new.product_id);
  ...
  call p_insert_update_delete_products_table(  v_delete_flag,
    ...
    CREATE TRIGGER t_Delete_online_futurebazaar_products BEFORE DELETE ON online_futurebazaar_products
    FOR EACH ROW
    BEGIN
 ...
 call p_insert_update_delete_products_table(    v_delete_flag,
  ...
  def down
	...
	execute "DELETE FROM online_futurebazaar_products"
	execute "call p_drop_online_vendor('futurebazaar')"
        execute "DROP TRIGGER t_Delete_online_futurebazaar_products"
        execute "DROP TRIGGER t_Update_online_futurebazaar_products"
        execute "DROP TRIGGER t_Insert_online_futurebazaar_products"
        execute "DROP TABLE online_futurebazaar_products"
        system "rails destroy model online_futurebazaar_products"
  end
end
