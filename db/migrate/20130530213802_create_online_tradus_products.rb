class CreateOnlineTradusProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,trial_flag,created_at) values('Tradus','/Images/Vendor_Logos/croma.png','An Online Shop','online','www.croma.com','feedback@gcroma.in','02131230324324','na', 'greendust,Nehrunagar,Ahmedabad. Gujarat. India - 380015.','na','na','na','na','na','COD',
'tablets_lists','purchase','0','n',now());
  ...
  CREATE TABLE IF NOT EXISTS online_tradus_products (
  ...
  SQL
    ...
    CREATE TRIGGER t_Insert_online_tradus_products AFTER INSERT ON online_tradus_products
    FOR EACH ROW
    BEGIN
	...
	SELECT vendor_id INTO v_VendorID FROM vendors_lists WHERE vendor_name='tradus';
    ...
    CREATE TRIGGER t_Update_online_tradus_products AFTER UPDATE ON online_tradus_products
    FOR EACH ROW
    BEGIN
...
SET v_DebugID = concat('uonlinev',v_VendorID,'p',new.product_id);
  ...
  call p_insert_update_delete_products_table(  v_delete_flag,
    ...
    CREATE TRIGGER t_Delete_online_tradus_products BEFORE DELETE ON online_tradus_products
    FOR EACH ROW
    BEGIN
 ...
 call p_insert_update_delete_products_table(    v_delete_flag,
  ...
  def down
	...
	execute "DELETE FROM online_tradus_products"
	execute "call p_drop_online_vendor('tradus')"
        execute "DROP TRIGGER t_Delete_online_tradus_products"
        execute "DROP TRIGGER t_Update_online_tradus_products"
        execute "DROP TRIGGER t_Insert_online_tradus_products"
        execute "DROP TABLE online_tradus_products"
        system "rails destroy model online_tradus_products"
  end
end
