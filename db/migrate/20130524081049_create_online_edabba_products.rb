class CreateOnlineEdabbaProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,trial_flag,created_at) values('edabba','/Images/Vendor_Logos/infibeam.png','An Online Shop','online','www.zoomin.com','feedback@infibeam.in','08049091234','na', '9th Floor,A-Wing,Gopal Palace,Nehrunagar,Ahmedabad. Gujarat. India - 380015.','na','na','na','na','na','COD',
'mobiles_lists','purchase','2012-07-07','n',now());
  ...
  CREATE TABLE IF NOT EXISTS online_edabba_products (
  ...
  SQL
    ...
    CREATE TRIGGER t_Insert_online_edabba_products AFTER INSERT ON online_edabba_products
    FOR EACH ROW
    BEGIN
	...
	SELECT vendor_id INTO v_VendorID FROM vendors_lists WHERE vendor_name='edabba';
    ...
    CREATE TRIGGER t_Update_online_edabba_products AFTER UPDATE ON online_edabba_products
    FOR EACH ROW
    BEGIN
...
SET v_DebugID = concat('uonlinev',v_VendorID,'p',new.product_id);
  ...
  call p_insert_update_delete_products_table(  v_delete_flag,
    ...
    CREATE TRIGGER t_Delete_online_edabba_products BEFORE DELETE ON online_edabba_products
    FOR EACH ROW
    BEGIN
 ...
 call p_insert_update_delete_products_table(    v_delete_flag,
  ...
  def down
	...
	execute "DELETE FROM online_edabba_products"
	execute "call p_drop_online_vendor('edabba')"
        execute "DROP TRIGGER t_Delete_online_edabba_products"
        execute "DROP TRIGGER t_Update_online_edabba_products"
        execute "DROP TRIGGER t_Insert_online_edabba_products"
        execute "DROP TABLE online_edabba_products"
        system "rails destroy model online_edabba_products"
  end
end
