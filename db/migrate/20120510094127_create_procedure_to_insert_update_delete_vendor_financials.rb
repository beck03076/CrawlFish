class CreateProcedureToInsertUpdateDeleteVendorFinancials < ActiveRecord::Migration
  def up
    ...
    call debug.debug_insert(v_DebugID,'Procedure p_Insert_update_delete_vendor_financials has been called');
    ...
    /* Insert a record in debug table for tracking the events */
    call debug.debug_insert(v_DebugID,concat('Calculated date difference is ',v_date_diff));


			SET v_date_end = v_subscribed_date + INTERVAL v_subscription_period DAY;

    /* Insert a record in debug table for tracking the events */
    call debug.debug_insert(v_DebugID,concat('Calculated date end is ',v_date_end));
