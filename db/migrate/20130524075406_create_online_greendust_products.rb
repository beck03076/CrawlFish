class CreateOnlineGreendustProducts < ActiveRecord::Migration
...
vendor_address,latitude,longitude,city_name,branch_name,working_time,miscellaneous,vendor_sub_categories,monetization_type,subscribed_date,trial_flag,created_at) values('Greendust','/Images/Vendor_Logos/greendust.png','An Online Shop','online','www.greendust.com','feedback@greendust.in','080490324324','na', 'greendust,Nehrunagar,Ahmedabad. Gujarat. India - 380015.','na','na','na','na','na','COD',
'desktops_lists,headsets_lists,headphones_lists,speakers_lists','purchase','0','n',now());

	SQL

  execute <<-SQL

  CREATE TABLE IF NOT EXISTS online_greendust_products (
  ...
  SQL
    ...
    CREATE TRIGGER t_Insert_online_greendust_products AFTER INSERT ON online_greendust_products
    FOR EACH ROW
    BEGIN
	...
	SELECT vendor_id INTO v_VendorID FROM vendors_lists WHERE vendor_name='greendust';
    ...
    CREATE TRIGGER t_Update_online_greendust_products AFTER UPDATE ON online_greendust_products
    FOR EACH ROW
    BEGIN
...
SET v_DebugID = concat('uonlinev',v_VendorID,'p',new.product_id);
  ...
  call p_insert_update_delete_products_table(  v_delete_flag,
    ...
    CREATE TRIGGER t_Delete_online_greendust_products BEFORE DELETE ON online_greendust_products
    FOR EACH ROW
    BEGIN
 ...
 call p_insert_update_delete_products_table(    v_delete_flag,
  ...
  def down
	...
	execute "DELETE FROM online_greendust_products"
	execute "call p_drop_online_vendor('greendust')"
        execute "DROP TRIGGER t_Delete_online_greendust_products"
        execute "DROP TRIGGER t_Update_online_greendust_products"
        execute "DROP TRIGGER t_Insert_online_greendust_products"
        execute "DROP TABLE online_greendust_products"
        system "rails destroy model online_greendust_products"
  end
end
