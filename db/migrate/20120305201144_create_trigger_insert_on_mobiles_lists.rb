class CreateTriggerInsertOnMobilesLists < ActiveRecord::Migration
  def up
    ...
    /* Insert a record in debug table for tracking the events */
    call debug.debug_insert(v_DebugID,'The table attributes is dropped successfully, about a nanosecond ago');

    ELSE

    /* Insert a record in debug table for tracking the events */
    call debug.debug_insert(v_DebugID,'The attributes table is not dropped successfully');
