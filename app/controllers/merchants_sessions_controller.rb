class MerchantsSessionsController < ApplicationController
  ...
  def create
	...
	if(@db_merchants.nil?)
		@message = "<span style='color:#C42525;font-weight:bold;font-size:14px;'>" + @login_name + "</span> is not registered as a merchant with CrawlFish. Please enter a valid login name"
		render('merchants_sessions/forgot_password')
	else
		if !(@db_merchants.vendor_id.nil?)
			...
			if @password_request.save
				TestMailer.after_create_merchants_password_requests.deliver
