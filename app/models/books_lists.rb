...
class BooksLists < ActiveRecord::Base
   ...
   def self.check_distinct_book_name_count(books_list_id_array)

     where(:books_list_id => books_list_id_array).select("distinct(book_name)").size
   ...
   def self.get_genre_ids(books_list_id)
    ...
    find(books_list_id).genres.where(:level_id => 1).map {|i| [i.genre_id,i.genre_name] }

   end

   def self.get_author_ids(books_list_id)

    find(books_list_id).authors.map {|i| [i.author_id,i.author_name] }

   end

   def self.get_publisher_ids(books_list_id)

    find(books_list_id).publishers.map {|i| [i.publisher_id,i.publisher] }
