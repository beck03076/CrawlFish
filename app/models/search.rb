class Search < ActiveRecord::Base
  ...
  def self.fetch_products_list_id(extended_params,products_list_id,column_name,table_name)
    ...
    puts "^^^^^^^^^^^^^^^^^^ #{extended_params}"

    products_list_ids_array = [ ]

    if (extended_params.empty?)

    products_list_ids_array << table_name.camelize.constantize.where(column_name+" IN (?)",products_list_id).select(column_name).order("FIELD ("+column_name+","+products_list_id.join(",")+")").map{|i| i.send(column_name) }
  ...
  def self.fetch_products_list_id_using_feature_id(feature,feature_id,sub_category_name,product_id,products_list_id_hash)
    ...
    if sub_category_name == "books_lists"
      ...
      if feature == "genre"
        ...
        products_list_ids = BooksF2Genre.get_products_list_id_from_feature_id(feature_id)

      elsif feature == "author"

        products_list_ids = BooksF1Author.get_products_list_id_from_feature_id(feature_id)

      elsif feature == "publisher"

        products_list_ids = BooksF7Publisher.get_products_list_id_from_feature_id(feature_id)

      end

    elsif sub_category_name == "mobiles_lists"

      if feature == "mobile_brand"

        products_list_ids = MobilesF1MobileBrand.get_products_list_id_from_feature_id(feature_id)

      elsif feature == "mobile_type"

        products_list_ids = MobilesF3MobileType.get_products_list_id_from_feature_id(feature_id)

      elsif feature == "mobile_design"

        products_list_ids = MobilesF4MobileDesign.get_products_list_id_from_feature_id(feature_id)

      elsif feature == "mobile_os_version"

        products_list_ids = MobilesF5OsVersion.get_products_list_id_from_feature_id(feature_id)
    ...
    (products_list_ids - [product_id]) & products_list_id_hash
    ...
    def self.construct_query(sortedFinal,products_list_ids_array,products_list_id,table_name)
      ...
      sortedFinal.each do |detail|
        ...
        tail = ") AND "+detail[:feature_type_id].to_s+" = "+detail[:id].to_s
        if sql.empty?
          firstSql = "SELECT "+products_list_id.to_s+" FROM "+detail[:link_table_name].to_s+" WHERE "+products_list_id.to_s+" IN ("+products_list_ids_array.join(",")+") AND "+detail[:feature_type_id].to_s+" ="+detail[:id].to_s
          sql = firstSql
        else
          sql = head+sql+tail
        end
