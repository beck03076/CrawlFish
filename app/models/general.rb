...
class General < ActiveRecord::Base
  ...
  def self.get_price_range(product_ids,sub_category_id,type = "both")
   ...
   if type == "both"
     ...
     sql = "SELECT products_list_id,
               MIN(lowest_price),
               MAX(highest_price),
               IF(MIN(lowest_price) = MAX(highest_price),1,0) AS equal_flag
               FROM

               ((SELECT products_list_id,
                      MIN(`local_grid_details`.`price`) AS lowest_price,
                      MAX(`local_grid_details`.`price`) AS highest_price
               FROM `local_grid_details`
               INNER JOIN link_products_lists_vendors ON
               link_products_lists_vendors.unique_id = local_grid_details.unique_id
               WHERE (link_products_lists_vendors.products_list_id IN
               (" + product_ids_string+")
               AND link_products_lists_vendors.sub_category_id = "+sub_category_id_string+" )
               GROUP BY link_products_lists_vendors.products_list_id)

               UNION

               (SELECT products_list_id ,
                       MIN(`online_grid_details`.`price`) AS lowest_price,
                       MAX(`online_grid_details`.`price`) AS highest_price
                FROM `online_grid_details`
                INNER JOIN link_products_lists_vendors ON
                link_products_lists_vendors.unique_id = online_grid_details.unique_id
                WHERE (link_products_lists_vendors.products_list_id IN
                (" + product_ids_string+")
                AND link_products_lists_vendors.sub_category_id = "+sub_category_id_string+")
                GROUP BY link_products_lists_vendors.products_list_id))
        ...
        sql = "SELECT products_list_id,
               ...
               (SELECT products_list_id ,
                       MIN(`"+type+"_grid_details`.`price`) AS lowest_price,
                       MAX(`"+type+"_grid_details`.`price`) AS highest_price
                FROM `"+type+"_grid_details`
                INNER JOIN link_products_lists_vendors ON
                link_products_lists_vendors.unique_id = "+type+"_grid_details.unique_id
                WHERE (link_products_lists_vendors.products_list_id IN
                (" + product_ids_string+")
                AND link_products_lists_vendors.sub_category_id = "+sub_category_id_string+")
                GROUP BY link_products_lists_vendors.products_list_id)
