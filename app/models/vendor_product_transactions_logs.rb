class VendorProductTransactionsLogs < ActiveRecord::Base
  ...
  def self.get_most_recognized_mobiles_impressions(vendor_id)
	
        results = []

	results <<    joins("INNER JOIN mobiles_lists").where("vendor_product_transactions_logs.vendor_id = ?
                            AND mobiles_lists.mobiles_list_id= vendor_product_transactions_logs.product_id
                            AND vendor_product_transactions_logs.sub_category_id = 2",vendor_id).group("vendor_product_transactions_logs.product_id,vendor_product_transactions_logs.sub_category_id").order("SUM(vendor_product_transactions_logs.page_impressions_count) DESC").select("mobiles_lists.mobile_name as mobile_name,mobiles_lists.mobile_color as mobile_color,SUM(vendor_product_transactions_logs.page_impressions_count) as impression_count").limit(3)
  ...
  def self.get_count_button_clicks(vendor_id)

    results =  where("vendor_id IN (?)",
                      vendor_id).select("vendor_id,
                      SUM(button_clicks_count) as clicks_no").group("vendor_id")
 ...
 def self.get_most_recognized_books_count(vendor_id)
	
        results = []

	results <<    joins("INNER JOIN books_lists").where("vendor_product_transactions_logs.vendor_id = ?
                            AND books_lists.books_list_id= vendor_product_transactions_logs.product_id
                            AND vendor_product_transactions_logs.sub_category_id = 1",vendor_id).group("vendor_product_transactions_logs.product_id,vendor_product_transactions_logs.sub_category_id").order("SUM(vendor_product_transactions_logs.button_clicks_count) DESC").select("books_lists.book_name as book_name,books_lists.isbn13 as isbn13,SUM(vendor_product_transactions_logs.button_clicks_count) as impression_count").limit(3)
 ...
 def self.get_most_recognized_mobiles_count(vendor_id)
	
        results = []

	results <<    joins("INNER JOIN mobiles_lists").where("vendor_product_transactions_logs.vendor_id = ?
                            AND mobiles_lists.mobiles_list_id= vendor_product_transactions_logs.product_id
                            AND vendor_product_transactions_logs.sub_category_id = 2",vendor_id).group("vendor_product_transactions_logs.product_id,vendor_product_transactions_logs.sub_category_id").order("SUM(vendor_product_transactions_logs.button_clicks_count) DESC").select("mobiles_lists.mobile_name as mobile_name,mobiles_lists.mobile_color as mobile_color,SUM(vendor_product_transactions_logs.button_clicks_count) as impression_count").limit(3)
 ...
 def self.get_most_recognized_books_impressions(vendor_id)
	
        results = []

	results <<    joins("INNER JOIN books_lists").where("vendor_product_transactions_logs.vendor_id = ?
                            AND books_lists.books_list_id= vendor_product_transactions_logs.product_id
                            AND vendor_product_transactions_logs.sub_category_id = 1",vendor_id).group("vendor_product_transactions_logs.product_id,vendor_product_transactions_logs.sub_category_id").order("SUM(vendor_product_transactions_logs.page_impressions_count) DESC").select("books_lists.book_name as book_name,books_lists.isbn13 as isbn13,SUM(vendor_product_transactions_logs.page_impressions_count) as impression_count").limit(3)
