namespace :merchant do
  ...
  desc "LOCAL:
        ...
        4. set login_type as admin or branch based on the args.
        5. Insert login_details into merchants table
        6. populate merchant_id in merchants_lists"

  task :approve_local,[:vendor_name,:city_name,:branch_name,:login_type]  => [:environment]   do |t,args|
    puts "-"
    puts "-"
    puts "-"
    ...
    login_type = set_login_type(args[:login_type])

    insert_into_merchants(login_name,password_hash,password_salt,table_name,args[:vendor_name].downcase,password,"local",vendor_id,monetization_type,args[:city_name].downcase,args[:branch_name].downcase,login_type)
  ...
  def insert_into_merchants(login_name,password_hash,password_salt,table_name,vendor_name,password,type,vendor_id,monetization_type,city_name = 0,branch_name = 0, login_type = "branch")
    ...
    if answer == 'y'
        ...
        sql = "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = "+"'"+table_name+"'"+" AND table_schema = "+"'"+db_name+"'"
          ...
          if count == 1
            ...
            puts ("TABLE "+table_name.upcase+" EXIST!! -- Be Alarmed -- You have created an individual table for this vendor.")

          else

            puts ("TABLE "+table_name.upcase+" DOES NOT EXIST!! -- Be Alarmed -- Since, it has been decided not to create individual tables for trial vendors!!!")

          end
          
              sql = "INSERT INTO merchants(login_name,password_hash,password_salt,table_name,business_type,vendor_id,login_type) VALUES('#{login_name}','#{password_hash}','#{password_salt}','#{table_name}','#{type}',#{vendor_id},'#{login_type}')"
    ...
    STDOUT.reopen(File.open(log_file_name, 'a+'))
    puts "-"
    puts "-"
    puts "-"
    ...
    if type == "local"
      ...
      puts "+--------------------------------------------"
      puts "|Login Type        : "+login_type.capitalize
